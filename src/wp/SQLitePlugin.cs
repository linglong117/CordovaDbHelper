...
namespace Cordova.Extension.Commands
{
    ...
    public class DbHelper : BaseCommand
    {
        #region DbHelper options

        [DataContract]
        public class DbHelperOpenCloseOptions
        {
        ...
        [DataContract]
        public class DbHelperExecuteSqlBatchArgs
        {
        ...
        [DataContract]
        public class DbHelperExecuteSqlBatchOptions
        {
            [DataMember(IsRequired = true, Name = "dbargs")]
            public DbHelperExecuteSqlBatchArgs dbargs { get; set; }
        ...
        public class TransactionsCollection : Collection<DbHelperTransaction>
        {
        ...
        private DbHelperOpenCloseOptions dbOptions = null;
        private SQLiteConnection db;

        public void open(string options)
        {
            string mycbid = this.CurrentCommandCallbackId;
            //System.Diagnostics.Debug.WriteLine("DbHelper.open() with cbid " + mycbid + " options:" + options);

            try
            {
                String [] jsonOptions = JsonHelper.Deserialize<string[]>(options);
                dbOptions = JsonHelper.Deserialize<DbHelperOpenCloseOptions>(jsonOptions[0]);
            ...
            if (dbOptions != null)
            {
                ...
                //System.Diagnostics.Debug.WriteLine("DbHelper.open() dbname:" + dbOptions.name);
        ...
        {
            string mycbid = this.CurrentCommandCallbackId;
            //System.Diagnostics.Debug.WriteLine("DbHelper.close() with cbid " + mycbid + " options:" + options);
        ...
        public void delete(string options)
        {
            ...
            //System.Diagnostics.Debug.WriteLine("DbHelper.delete() with cbid " + mycbid + " options:" + options);
            try
            {
                String[] jsonOptions = JsonHelper.Deserialize<string[]>(options);
                mycbid = jsonOptions[1];
            ...
            //DispatchCommandResult(new PluginResult(PluginResult.Status.ERROR, "Sorry DbHelper.delete() not implemented"), mycbid);
            DispatchCommandResult(new PluginResult(PluginResult.Status.OK), mycbid);
        }
        ...
        {
            string mycbid = this.CurrentCommandCallbackId;
            //System.Diagnostics.Debug.WriteLine("DbHelper.backgroundExecuteSqlBatch() with cbid " + mycbid + " options:" + options);
        ...
        {
            string mycbid = this.CurrentCommandCallbackId;
            //System.Diagnostics.Debug.WriteLine("DbHelper.executeSqlBatch() with cbid " + mycbid + " options:" + options);

            //Deployment.Current.Dispatcher.BeginInvoke(() =>
            {
                List<string> opt = JsonHelper.Deserialize<List<string>>(options);
                DbHelperExecuteSqlBatchOptions batch = JsonHelper.Deserialize<DbHelperExecuteSqlBatchOptions>(opt[0]);
                ...
                foreach (DbHelperTransaction transaction in batch.executes)
                {
					string resultString = "";
